...
os.environ["HS_EXPERIMENTAL_SEMANTIC_TIEBREAKER"] = "1"

sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
import utils.payoff_engine as pe

engine = pe.PayoffEngine()
hook = {"start": 0.0, "end": 5.0, "text": "The biggest mistake I ever made..."}
...
spans = [
    ...
    {"name": "I was wrong.", "text": "I was wrong.", "start": 45.0, "end": 50.0, "idxs": [2]},
    {"name": "I stopped caring.", "text": "I stopped caring.", "start": 45.0, "end": 50.0, "idxs": [3]},
]

scored_candidates = []
for s in spans:
    score = engine._score_span(s, thread, hook, promise_frame)
    score.information_gain = pe._measure_narrative_resolution(score.text, hook["text"])
    scored_candidates.append(score)

scored_candidates.sort(key=lambda c: (round(c.final_score, 2), c.information_gain, c.boundary_crispness), reverse=True)

print("--- EXPERIMENTAL SEMANTIC RESOLUTION RANKING ---")
for i, c in enumerate(scored_candidates):
    print(f"{i+1}. {c.text}")
    print(f"   Narrative Resolution (Information Gain): {c.information_gain:.3f}")
